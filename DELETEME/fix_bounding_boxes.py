...
# this script changes BoundingBoxes serializations so that their min values are actual the min values, and their max values are actually their max values

def fix_bounding_box(box: BoundingBox) -> BoundingBox:
    return BoundingBox(
        min(box.minX, box.maxX),
        min(box.minY, box.maxY),
        min(box.minZ, box.maxZ),
        max(box.minX, box.maxX),
        max(box.minY, box.maxY),
        max(box.minZ, box.maxZ)
    )
...
for n in range(0, len(old_checkpoints)):
    ...
    new_detection_box = fix_bounding_box(old_detection_box)
