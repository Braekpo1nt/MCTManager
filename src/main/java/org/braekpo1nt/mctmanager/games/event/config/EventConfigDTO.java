...
/**
 * @param title       the title of the event, used in the sidebar and for
 *                    announcing the winner
 * @param multipliers must have at least one element. The nth multiplier is used
 *                    on the nth game in the event. If there are x multipliers,
 *                    and we're on game z where z is greater than x, the xth
 *                    multiplier is used. A multiplier will be multiplied by all
 *                    points awarded during it's paired game.
 * @param durations   various durations during the event
 * @param tips        list of game tips displayed during WaitingInHubState
 */
record EventConfigDTO(
    ...
    public void validate(@NotNull Validator validator) {
        ...
        validator.validate(Main.VALID_CONFIG_VERSIONS.contains(this.version), "invalid config version (%s)",
                this.version);
        ...
        validator.validate(this.tips.tips().size() >= 1, "there must be at least 1 game tip");
     ...
     * 
     * @param waitingInHub  the time spent waiting in the hub between games
     *                      (seconds)
     * @param halftimeBreak the duration of the halftime break (seconds)
     * @param voting        the duration of the voting phase (seconds)
     * @param startingGame  the delay to start the game after the voting phase
     *                      (seconds)
     * @param backToHub     the delay after a game is over before returning to the
     *                      hub (seconds)
     */
