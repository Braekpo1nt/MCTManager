...
public class CaptureTheFlagUtils {
     ...
     * @throws IndexOutOfBoundsException if teamIds.size() is 1 or less
     */
    public static @NotNull List<MatchPairing> generateMatchPairings(@NotNull List<String> teamIds) {
        List<MatchPairing> combinations = new ArrayList<>();
        for (int i = 0; i < teamIds.size(); i++) {
            for (int j = i + 1; j < teamIds.size(); j++) {
                String northTeam = teamIds.get(i);
                String southTeam = teamIds.get(j);
